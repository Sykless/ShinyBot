...
function decryptPokemonData()
    ...
    pid = memory.read_u32_le(pidAddress)
    checksum = memory.read_u16_le(pidAddress + 6)
...
function decryptBlockA()
    ...
    pokemon["PokedexID"] = decryptData(pidAddress + BlockAoffset + START)
    pokemon[" Name "] = POKEMON_NAMES[pokemon["PokedexID"]]

    -- Held Item (0x0A-0x0B)
    pokemon["HeldItem"] = decryptData(pidAddress + BlockAoffset + START + 2)

    -- OT ID (0x0C-0x0D)
    pokemon["OT"] = decryptData(pidAddress + BlockAoffset + START + 4)

    -- OT Secret ID (0x0E-0x0F)
    pokemon["OTSecretID"] = decryptData(pidAddress + BlockAoffset + START + 6)

    -- Experience points (0x10-0x13)
    pokemon["Experience"] = decryptData(pidAddress + BlockAoffset + START + 8)
    ...
    friendship_ability = decryptData(pidAddress + BlockAoffset + START + 12)
    ...
    markings_originalLanguage = decryptData(pidAddress + BlockAoffset + START + 14)
    ...
    HpEv_AtkEv = decryptData(pidAddress + BlockAoffset + START + 16)
    ...
    DefEv_SpeEv = decryptData(pidAddress + BlockAoffset + START + 18)
    ...
    cuteSmartContest = decryptData(pidAddress + BlockAoffset + START + 24)
    ...
    toughSheenContest = decryptData(pidAddress + BlockAoffset + START + 26)
    ...
    pokemon["SinnohRibbon3"] = decryptData(pidAddress + BlockAoffset + START + 28)

    -- Sinnoh Ribbon Set 2 (0x26-0x27)
    pokemon["SinnohRibbon4"] = decryptData(pidAddress + BlockAoffset + START + 30)
end
...
function decryptBlockB()
    ...
    -- Move 1 ID (0x28-0x29)
    pokemon["Move1"] = MOVE_NAMES[decryptData(pidAddress + BlockBoffset + START)]

    -- Move 2 ID (0x2A-0x2B)
    pokemon["Move2"] = MOVE_NAMES[decryptData(pidAddress + BlockBoffset + START + 2)]

    -- Move 3 ID (0x2C-0x2D)
    pokemon["Move3"] = MOVE_NAMES[decryptData(pidAddress + BlockBoffset + START + 4)]

    -- Move 4 ID (0x2E-0x2F)
    pokemon["Move4"] = MOVE_NAMES[decryptData(pidAddress + BlockBoffset + START + 6)]

    -- Move 1-2 Current PP (0x30-0x31)
    move12pp = decryptData(pidAddress + BlockBoffset + START + 8)
    pokemon["Move1PP"] = getBits(move12pp, 0, 8)
    pokemon["Move2PP"] = getBits(move12pp, 8, 8)

    -- Move 3-4 Current PP (0x32-0x33)
    move34pp = decryptData(pidAddress + BlockBoffset + START + 10)
    pokemon["Move3PP"] = getBits(move34pp, 0, 8)
    pokemon["Move4PP"] = getBits(move34pp, 8, 8)

    -- Move PP Ups (0x34-0x37)
    pokemon["MovePPUps"] = decryptData(pidAddress + BlockBoffset + START + 12)

    -- Remaining two bytes from Move PP Ups (0x36-0x37)
    prng = nextRecursivePrng(prng) -- Still need to update PRNG if we miss data

    -- First two bytes of Individual Values (0x38-0x39)
    IVpart1 = decryptData(pidAddress + BlockBoffset + START + 16)

    -- Last two bytes of Individual Values (0x3A-0x3B)
    IVpart2 = decryptData(pidAddress + BlockBoffset + START + 18)
    ...
    pokemon["HoennRibbon1"] = decryptData(pidAddress + BlockBoffset + START + 20)

    -- Hoenn Ribbon Set 2 (0x3E-0x3F)
    pokemon["HoennRibbon2"] = decryptData(pidAddress + BlockBoffset + START + 22)

    data = decryptData(pidAddress + BlockBoffset + START + 24)
    pokemon["Female"] = getBits(data,1,1)
    pokemon["Genderless"] = getBits(data,2,1)
    pokemon["AlternateForms"] = getBits(data,3,5)

    -- Unused (0x42-0x43)
    unused = decryptData(pidAddress + BlockBoffset + START + 26)

    -- Egg Location (0x44-0x45)
    pokemon["EggLocation"] = decryptData(pidAddress + BlockBoffset + START + 28)

    -- Met at Location (0x46-0x47)
    pokemon["MetLocation"] = decryptData(pidAddress + BlockBoffset + START + 30)
end
...
function decryptBlockC()
    ...
    for i = 0, 10 do
        ...
        if (searchNickname) then
            nicknameLetter = decryptData(pidAddress + BlockCoffset + START + (i*2))
    ...
    pokemon["OriginGame"] = getBits(decryptData(pidAddress + BlockCoffset + START + 22), 8, 8)

    -- Sinnoh Ribbon Set 3 (0x60-0x61)
    pokemon["SinnohRibbon3"] = decryptData(pidAddress + BlockCoffset + START + 24)

    -- Sinnoh Ribbon Set 4 (0x62-0x63)
    pokemon["SinnohRibbon4"] = decryptData(pidAddress + BlockCoffset + START + 26)
...
function decryptBlockD()
    ...
    for i = 0, 7 do
        ...
        if (searchOTName) then
            nameLetter = decryptData(pidAddress + BlockDoffset + START + (i*2))
    ...
    yearMonth_Egg = decryptData(pidAddress + BlockDoffset + START + 16)

    -- Remaining byte of Date Egg Received and first byte of Date Met (0x7A-0x7B)
    dayEgg_yearMet = decryptData(pidAddress + BlockDoffset + START + 18)

    -- Date Met (0x7B-0x7D)
    monthDay_Met = decryptData(pidAddress + BlockDoffset + START + 20)
    ...
    pokemon["EggLocationDP"] = decryptData(pidAddress + BlockDoffset + START + 22)

    -- Met Location (Diamond/Pearl) (0x80-0x81)
    pokemon["MetLocationDP"] = decryptData(pidAddress + BlockDoffset + START + 24)

    -- Pokérus - Poké Ball (0x82-0x83)
    pokerusPokeball = decryptData(pidAddress + BlockDoffset + START + 26)
    ...
    metLevelFemaleOT = decryptData(pidAddress + BlockDoffset + START + 28)
    ...
    -- pokemon["PokeballHGSS"] = getBits(decryptData(pidAddress + BlockDoffset + START + 30), 0, 8)
end
...
function decryptStats()
    ...
    -- Status (0x88)
    status = getBits(decryptData(pidAddress + 0x88),0,8)
    ...
    levelCapsule = decryptData(pidAddress + 0x8C)
    pokemon["Level"] = getBits(levelCapsule, 0, 8)
    pokemon["Capsule"] = getBits(levelCapsule, 8, 8)

    -- Current HP (0x8E-0x8F)
    pokemon["HPCurrent"] = decryptData(pidAddress + 0x8E)

    -- Max HP (0x90-0x91)
    pokemon["HPMax"] = decryptData(pidAddress + 0x90)

    -- Attack (0x92-0x93)
    pokemon["Attack"] = decryptData(pidAddress + 0x92)

    -- Defense (0x94-0x95)
    pokemon["Defense"] = decryptData(pidAddress + 0x94)

    -- Speed (0x96-0x97)
    pokemon["Speed"] = decryptData(pidAddress + 0x96)

    -- Special Attack (0x98-0x99)
    pokemon["SpecialAttack"] = decryptData(pidAddress + 0x98)

    -- Special Defense (0x9A-0x9B)
    pokemon["SpecialDefense"] = decryptData(pidAddress + 0x9A)
    ...
    pokemon["SealCoordinates"] = decryptData(pidAddress + 0xD4)
end