...
-- Main loop : never stop the script
while true do
    ...
    while not isGameRunning() do
        ...
        refreshPID() -- Refresh baseAddress
    end
    ...
    comm.mmfWrite("joypad", string.rep("\x00", 8192))
    comm.mmfWrite("gameData", string.rep("\x00", 4096))
    comm.mmfWrite("wildPokemonData", string.rep("\x00", 2048))
    comm.mmfWrite("playerData", string.rep("\x00", 256))
    comm.mmfWrite("runSections", string.rep("\x00", 256))
    comm.mmfWrite("specialPokemon", string.rep("\x00", 256))
    comm.mmfWrite("titleScreen", string.rep("\x00", 8))
    ...
    while isGameRunning() do
        ...
        -- Check if we're in-game or before title screen
        titleScreen = isOnTitleScreen()
        comm.mmfWrite("titleScreen", tostring(titleScreen and 1 or 0) .. "\x00")

        -- Set Repel steps to a fixed number to prevent it from decreasing
        if (SKIP_ENCOUNTERS and not titleScreen) then
